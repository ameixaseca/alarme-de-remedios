      ...
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          ...
          <div className="flex gap-2.5 pt-1">
            <button type="button" onClick={onClose} className="flex-1 border border-gray-300 rounded-lg py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">Cancelar</button>
            <button type="submit" disabled={loading} className="flex-1 bg-indigo-600 text-white rounded-lg py-2.5 text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50 transition-colors">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-start gap-3 min-w-0">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${isOverdue ? "bg-red-100" : "bg-amber-100"}`}>
            {isOverdue ? <IconAlertTriangle className="w-4 h-4 text-red-600" /> : <IconClock className="w-4 h-4 text-amber-600" />}
          </div>
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-400 mt-0.5">{item.dose_fraction ?? item.dose_quantity} {item.dose_unit}</p>
          </div>
        </div>
        <span className={`shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full ${isOverdue ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>
        ...
        <p className="text-xs text-gray-500">
          ...
          {isOverdue && item.minutes_overdue && (
            <span className="text-red-500 ml-1">
              · {item.minutes_overdue >= 60 ? `${Math.floor(item.minutes_overdue / 60)}h ${item.minutes_overdue % 60}min atrás` : `${item.minutes_overdue}min atrás`}
            </span>
          )}
        </p>
        <button onClick={onApply} className="flex items-center gap-1.5 bg-indigo-600 text-white text-xs font-semibold px-3.5 py-1.5 rounded-lg hover:bg-indigo-700 transition-colors">
    ...
    <div className="space-y-5">
      <div>
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400 bg-gray-100 px-2.5 py-1 rounded-full">atualiza em {countdown}s</span>
        </div>
        {overdueCount > 0 && (
