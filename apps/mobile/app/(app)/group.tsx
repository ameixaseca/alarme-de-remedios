...
import {
  ...
  Alert, Modal, Pressable, RefreshControl, Share,
  ActivityIndicator,
} from 'react-native';
...
import {
  IconCopy, IconRefresh, IconTrash, IconPlus, IconGroup,
...
function OnboardingView({ onJoined }: { onJoined: () => void }) {
  ...
  async function handleCreate() {
    if (!groupName.trim()) { Alert.alert('Atenção', 'Informe um nome para o grupo.'); return; }
    setLoading(true);
    try {
  ...
  async function handleJoin() {
    if (!inviteCode.trim()) { Alert.alert('Atenção', 'Informe o código de convite.'); return; }
    setLoading(true);
    try {
  ...
  if (mode === 'create') {
    return (
      <View className="flex-1 bg-gray-50 px-6 justify-center">
        ...
        <Text className="text-gray-500 mb-6">Crie um grupo para gerenciar medicamentos com sua equipe.</Text>
  ...
  if (mode === 'join') {
    return (
      <View className="flex-1 bg-gray-50 px-6 justify-center">
        ...
        <Text className="text-gray-500 mb-6">Insira o código de convite compartilhado pelo administrador.</Text>
        <Text className="text-sm font-medium text-gray-700 mb-1">Código de convite</Text>
        <TextInput
          className="border border-gray-300 rounded-lg px-4 py-3 text-gray-800 bg-white mb-4 text-center text-lg font-bold uppercase"
  ...
  return (
    <View className="flex-1 bg-gray-50 px-6 justify-center">
      <View className="items-center mb-8">
        ...
        <Text className="text-2xl font-bold text-gray-800 text-center">Você não está em nenhum grupo</Text>
        <Text className="mt-2 text-gray-500 text-center">Crie um novo grupo ou entre em um existente.</Text>
      </View>
      <Button label="Criar novo grupo" onPress={() => setMode('create')} className="mb-3" />
      <Button label="Entrar com código de convite" onPress={() => setMode('join')} variant="secondary" />
    </View>
  );
}
    ...
    <View className="flex-row items-center justify-between px-4 py-3 border-b border-gray-100">
      <View className="flex-row items-center gap-3">
        <View className="w-9 h-9 rounded-full bg-indigo-100 items-center justify-center">
          <Text className="text-indigo-700 font-bold text-sm">{member.user.name[0]?.toUpperCase()}</Text>
        </View>
        <View>
      ...
      <View className="flex-row items-center gap-2">
        <View className={`rounded-full px-2 py-0.5 ${member.role === 'admin' ? 'bg-indigo-100' : 'bg-gray-100'}`}>
          <Text className={`text-xs font-medium ${member.role === 'admin' ? 'text-indigo-700' : 'text-gray-600'}`}>
        ...
        {isAdmin && !isCurrentUser && (
          <TouchableOpacity
            className="p-1"
            onPress={() => Alert.alert('Remover membro', `Remover ${member.user.name} do grupo?`, [
...
function GroupManagementView({ group }: { group: Group }) {
  ...
  async function copyInviteCode() {
    try {
      // expo-clipboard not available in all builds; fallback to Share
      await Share.share({ message: currentGroup.inviteCode });
    } catch { /* ignore */ }
  }
  ...
  async function regenerateCode() {
    Alert.alert('Regenerar código', 'O código atual deixará de funcionar. Continuar?', [
      { text: 'Cancelar', style: 'cancel' },
      {
    ...
    >
      ...
      <Card className="mx-4 mb-4 px-4 py-4">
        ...
        <View className="flex-row items-center gap-3 mb-3">
          ...
          {isAdmin && (
            <TouchableOpacity onPress={regenerateCode} disabled={regenerating}>
              {regenerating
...
export default function GroupScreen() {
  const { activeGroup, isLoading, refreshGroups } = useGroup();

  if (isLoading) return <LoadingSpinner fullScreen />;
  if (!activeGroup) return <OnboardingView onJoined={refreshGroups} />;
